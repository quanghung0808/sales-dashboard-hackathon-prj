...
export default function PublicApiDocsPage() {
  ...
  const apiKey = 'sak_live_jemmia_8f92a10b4c7e901a2b3c';
  ...
  const getSDKCode = () => {
    ...
    if (activeLang === 'curl') {
      return isGet
        ? `curl -X GET "${url}" -H "Authorization: Bearer ${apiKey}"`
...
console.log(data);`;
    ...
    if (activeLang === 'python') {
      return isGet
        ? `import requests
res = requests.get("${url}", headers={"Authorization": "Bearer ${apiKey}"})
print(res.json())`
        : `import requests
res = requests.${activeSec.method.toLowerCase()}("${url}", json=${bodyStr.replace(/true/g, 'True').replace(/false/g, 'False')}, headers={"Authorization": "Bearer ${apiKey}"})
print(res.json())`;
    }
      ...
      <div className="mb-6 flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <BookOpen className="h-5 w-5 text-blue-600" /> Tài Liệu REST API - Khách Hàng 360° & Hạng VIP
          </h1>
          <p className="text-xs text-slate-500">Bao gồm đầy đủ API cho Khách Hàng 360°, Hạng Thành Viên VIP/Gold/Silver, Timeline Events & Đơn Hàng</p>
        </div>
        <Badge variant="success" className="text-xs">Public REST API v2.0</Badge>
      </div>
          ...
          {activeSec.id === 'auth' && (
            <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4">
              ...
              <p className="text-xs text-slate-500 leading-relaxed">
                Tất cả HTTP request gửi tới REST API đều phải đính kèm Header Authorization mang giá trị Bearer Token do Admin cấp:
              </p>
